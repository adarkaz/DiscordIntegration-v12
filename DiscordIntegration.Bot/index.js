...
const discordClient = new discord.Client({ allowedMentions: { parse: [] } });
...
console.log(JSON.stringify({action: 'executeCommand', parameters: {channelId: message.channel.id, content: command, user: {id: message.author.id + '@discord', name: message.author.tag}}}) + '\0');
  sockets.forEach(socket => socket.write(JSON.stringify({action: 'executeCommand', parameters: {channelId: message.channel.id, content: command, user: {id: message.author.id + '@discord', name: `<@${message.author.id}>`}}}) + '\0'));
});
